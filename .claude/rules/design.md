...
#### fromJson/toJsonカスタム生成

json_serializableは`JsonOptional<T>`型を認識できないため、
`@Freezed(fromJson: false, toJson: false)`を指定し、fromJson/toJsonの両方を手動生成する。
`.g.dart` partディレクティブは不要（json_serializable非使用）。
...
`json.containsKey()`でキーの存在を判定し、3状態を正しく復元する：
- キーが存在しない → `JsonOptional.absent()`（`@Default`で自動適用）
